...
SELECT nro_alumno, nom_alumno, cod_carrera, nom_carrera, promedio
FROM (
SELECT nro_alumno, nom_alumno, cod_carrera, nom_carrera, AVG(nota_examen) AS promedio
FROM ALUMNOS, CARRERAS, MATRICULAS, EXAMENES
...
GROUP BY nro_alumno, nom_alumno, cod_carrera, nom_carrera
HAVING COUNT(*) > 20
) AS T
WHERE promedio >= 7;
...
SELECT COUNT(*)
FROM EXAMENES, ALUMNOS, CARRERAS
...
SELECT nro_alumno, nom_alumno
FROM ALUMNOS, MATRICULAS, EXAMENES, MATERIAS
WHERE ALUMNOS.nro_alumno = MATRICULAS.nro_alumno
AND MATRICULAS.nro_alumno = EXAMENES.nro_alumno
AND MATRICULAS.cod_carrera = EXAMENES.cod_carrera
...
GROUP BY nro_alumno, nom_alumno
HAVING COUNT(*) = (
SELECT COUNT(*)
FROM MATERIAS
